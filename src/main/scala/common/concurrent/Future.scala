...
trait Future[V] extends java.util.concurrent.Future[V] {
  ...
  def await(timeout: Long, unit: TimeUnit): Future[V]
  def await(timeoutMills: Long): Future[V]
  def awaitUninterruptibly(timeoutMills: Long, unit: TimeUnit): Future[V]
  def awaitUninterruptibly(timeoutMills: Long): Future[V]
